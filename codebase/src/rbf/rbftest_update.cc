...
void readRecord(RecordBasedFileManager *rbfm, const RID& rid, string str)
{
	...
	cout << endl << "Returned Data:" <<rid.slotNum<< endl;
...
int RBFTest_Update(RecordBasedFileManager *rbfm)
{
	...
	RID shortRID = rid;
	cout<<"shortRID.slotNum is  "<<shortRID.slotNum<<endl;

	// Insert mid record
	insertRecord(rbfm, rid, midstr);
	RID midRID = rid;
	cout<<"midRID.slotNum is  "<<midRID.slotNum<<endl;
	// Insert long record
	insertRecord(rbfm, rid, longstr);
	RID longRID = rid;
	cout<<"longRID.slotNum is  "<<longRID.slotNum<<endl;
	// update short record
	...
	insertRecord(rbfm, rid, longstr);
	cout<<"the 4th insertion is long, slotNum is  "<<rid.slotNum<<endl;
	insertRecord(rbfm, rid, longstr);
	cout<<"the 5th insertion is long, slotNum is  "<<rid.slotNum<<endl;
	// read mid record and verify its content
	readRecord(rbfm, midRID, midstr);

	// update short record
	updateRecord(rbfm, shortRID, longstr);//this one fail
